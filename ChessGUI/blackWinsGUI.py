...
def draw_menu():
    screen.fill(GREEN)
    draw_text("Black Wins!", FONT, BLACK, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50)
...
def drawMainMenu():
    ...
    img = pygame.image.load('ChessGUI/images/Logo.png').convert()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if button_rect.collidepoint(mouse_pos):
                ...
                if not drawMainMenu(): # Call the function to draw the other GUI
                    draw_menu()
